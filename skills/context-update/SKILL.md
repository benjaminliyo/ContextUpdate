---
name: context-update
description: Use when a conversation has produced decisions, corrections, or facts that may contradict statements written in reusable-context files (CLAUDE.md, AGENTS.md, ~/.claude/CLAUDE.md, implementation plans, personal instructions); when the user reverses an earlier preference; when wrapping up a session that materially changed configuration, architecture, or workflow conventions; when a new decision supersedes an older one recorded in a memory or rules file
---
